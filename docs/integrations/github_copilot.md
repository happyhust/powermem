...
Connect GitHub Copilot to PowerMem with the config generated by the [PowerMem VS Code extension](../../apps/vscode-extension/). The extension writes `~/.github/copilot/powermem.json`.
...
The agent follows [`../../apps/vscode-extension/SETUP.md`](../../apps/vscode-extension/SETUP.md), prefers a reusable `powermem-server` HTTP API backend, and updates GitHub Copilot's config only when Copilot is the current target.
